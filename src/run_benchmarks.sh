...
N_VALUES=(25 150 500 1000) # The N values to test
...
for N in "${N_VALUES[@]}"; do
  ...
  # 2. Generate code for the current N
  print_info "Step 2: Generating code for N=$N..."
  ...
  print_info "Step 3: Building benchmark project for N=$N..."
  ...
  # 4. Run BenchmarkDotNet, filtering for the current N
  print_info "Step 4: Running benchmarks for N=$N..."
  # Match Namespace.ClassName.*(N: Value)
  FILTER_PATTERN="Mediator.Switch.Benchmark.MediatorBenchmarks.*(N: $N)"  ARTIFACT_PATH_N="$OUTPUT_ARTIFACTS_BASE_DIR/N$N"
  ...
  print_success "Completed processing for N = $N"
done
